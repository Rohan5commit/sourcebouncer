...
import { Shield, Cpu, Coins, ArrowRight, GitBranch, Zap, Lock, Globe } from "lucide-react";

const BLOCKS = [
  {
    icon: <Globe className="w-6 h-6" />,
    title: "Discovery",
    desc: "Agent finds SourceBouncer on CROO Agent Store. Reviews pricing, capabilities, and ratings.",
  },
  {
    icon: <Lock className="w-6 h-6" />,
    title: "CAP Negotiation",
    desc: "Requester agent initiates CAP negotiation. Terms, pricing, and SLA agreed.",
  },
  {
    icon: <Coins className="w-6 h-6" />,
    title: "USDC Escrow",
    desc: "Funds locked in smart contract escrow on Base. Payment only released on verified delivery.",
  },
  {
    icon: <Cpu className="w-6 h-6" />,
    title: "NVIDIA NIM Verification",
    desc: "Claims analyzed against sources using AI inference. Structured verdicts produced.",
  },
  {
    icon: <Shield className="w-6 h-6" />,
    title: "Trust Report",
    desc: "Machine-readable evidence package with claim verdicts, scores, and reasoning.",
  },
  {
    icon: <Zap className="w-6 h-6" />,
    title: "Settlement & Reputation",
    desc: "Payment released on-chain. Agent reputation updated. Downstream agents consume output.",
  },
];
        ...
        <div className="flex items-center gap-4 flex-wrap">
          {["Research Agent", "Trading Agent", "Content Agent", "News Agent", "Any CAP Agent"].map((name) => (
            <div key={name} className="flex items-center gap-2">
              <div className="bg-blue-600/10 border border-blue-600/20 rounded-lg px-4 py-2 text-sm text-blue-300">
                {name}
              </div>
              <ArrowRight className="w-4 h-4 text-[#6b7280]" />
              <div className="bg-blue-600/20 rounded-lg px-4 py-2 text-sm text-white font-medium">
                SourceBouncer
              </div>
            </div>
          ))}
